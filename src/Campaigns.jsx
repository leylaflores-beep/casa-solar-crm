...
export function CampaignsView({ campaigns, contactos, currentUser, onChange, onRecover }) {
  ...
  return <div><div className="page-head row"><div><h2>Campañas</h2><p>Fidelización, promociones y seguimiento de beneficios.</p></div><div className="row">{isContactBoss(currentUser) && onRecover && <button className="btn-ghost" onClick={async()=>{try{const result=await onRecover();setRecoveryMessage(result.recovered ? `Se recuperaron ${result.recovered} campaña${result.recovered===1?"":"s"}. Total disponible: ${result.total}.` : `No se encontraron campañas faltantes. Hay ${result.total} disponibles.`);}catch(error){console.error(error);setRecoveryMessage("No se pudo completar la recuperación. Revisa la conexión e inténtalo nuevamente.");}}}><RefreshCw size={15}/> Recuperar campañas</button>}<button className="btn-primary" onClick={()=>setShowCreate(true)}><Plus size={16}/> Nueva campaña</button></div></div>
    {recoveryMessage && <div className="privacy-note">{recoveryMessage}</div>}
